...
func main() {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		for _, m := range msgs {
			...
			// file that exists relative to the export's own top level —
			// this is exactly the "does the JSON's uri match the real
			// folder layout" check worth doing against a real export.
			if m.MediaURL != nil {
